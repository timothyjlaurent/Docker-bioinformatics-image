...
#RUN apt-get install python-software-properties -y -q && add-apt-repository ppa:fkrull/deadsnakes -y && apt-get update && apt-get install python3.3 zip python-pip -y -q && apt-get cleanup
#
#	fab -f fabfile.py -H localhost -c install_biolinux:cleanup ;\
...
#RUN wget https://www.ebi.ac.uk/~zerbino/velvet/velvet_1.2.10.tgz && tar -xaf velvet_1.2.10.tgz && rm velvet_1.2.10.tgz ;\
#   cd velvet_1.2.10 ;\
#    make 'OPENMP=1' 'BIGASSEMBLY=1' 'MAXKMERLENGTH=99'  && ln -f -s `pwd`/* /usr/local/bin/pstorm
